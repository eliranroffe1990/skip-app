...
// ===== Diagnostics overlay (מציג שגיאות בצורה ברורה) =====
(function(){
  var b = document.getElementById('_diag');
  function show(m){
    try{
      ...
      p.textContent = m;
      b.appendChild(p);
      console.error('[SKIPAPP]', m);
    }catch(e){
      console.error('Diagnostics failed', e);
    }
  }
  window.addEventListener('error', function(e){ show(e.message || e.error || 'Error'); });
  window.addEventListener('unhandledrejection', function(e){ show('Promise: ' + (e.reason && e.reason.message || e.reason || 'unhandledrejection')); });
  window._diag_show = show;
})();

// ===== טקסטים (עברית/אנגלית) =====
var strings = {
  he: {
    continue:'המשך', save_next:'שמור והמשך', home:'בית', back:'חזרה',
    add_skip:'הוסף דילוג', history:'היסטוריה', insights:'תובנות',
    investments:'אפיקי השקעה', settings:'הגדרות', charts:'גרפים',
    amount:'סכום (₪)', description:'תיאור', category:'קטגוריה', save:'שמור',
    reset_all:'איפוס והרשמה מחדש',
    welcome_home:'ברוך שובך', register:'הרשמה', name:'שם', age:'גיל',
    goal:'יעד חיסכון חודשי (₪)', purpose:'מטרת חיסכון', language:'שפה',
    ...
    start:'התחל', go_home:'עבור למסך הבית', go_register:'עבור להרשמה',
    total:'שווי חיסכון מלא: ₪', terms_title:'תנאי שימוש',
    terms_text:'האפליקציה נועדה לשימוש חינוכי ולמידה על חסכון כסף.',
    chart_mode_line:'קו', chart_mode_bar:'עמודות', dashboard:'דאשבורד חכם',
    skips_count:'מספר דילוגים', avg_skip:'דילוג ממוצע', month_total:'חיסכון חודשי',
    export_pdf:'ייצוא PDF', purpose_opts:['השקעות','קניית נכס','חיסכון חכם','אחר']
  },
  en: {
    continue:'Continue', save_next:'Save & Continue', home:'Home', back:'Back',
    add_skip:'Add Skip', history:'History', insights:'Insights',
    investments:'Investments', settings:'Settings', charts:'Charts',
    amount:'Amount (₪)', description:'Description', category:'Category', save:'Save',
    reset_all:'Reset & Re-register',
    welcome_home:'Welcome back', register:'Register', name:'First name', age:'Age',
    goal:'Monthly saving goal (₪)', purpose:'Saving purpose', language:'Language',
    ...
    start:'Start', go_home:'Go to Home', go_register:'Go to Register',
    total:'Total saved: ₪', terms_title:'Terms of Use',
    terms_text:'This app is intended for educational use to learn about money saving.',
    chart_mode_line:'Line', chart_mode_bar:'Bar', dashboard:'Smart Dashboard',
    skips_count:'Skips', avg_skip:'Average Skip', month_total:'Monthly Savings',
    export_pdf:'Export PDF', purpose_opts:['Investments','Buy property','Smart saving','Other']
  }
};
...
function formatDate(iso){
  try{
    var d = new Date(iso);
    if(isNaN(d)) return '—';
    return d.toLocaleDateString(App.state.lang==='he'?'he-IL':'en-US');
  }catch(e){ return '—'; }
}
...
var App = {
  ...
  t:function(k){
    try{ return (strings[this.state.lang] || strings.he)[k] || k; }
    catch(e){ window._diag_show('t() failed: '+e); return k; }
  },

  persist:function(){
    try{
      localStorage.setItem('skipapp_user', JSON.stringify(this.state.user||null));
      localStorage.setItem('skipapp_data', JSON.stringify(this.state.data||[]));
      localStorage.setItem('skipapp_lang', this.state.lang);
      localStorage.setItem('skipapp_theme', this.state.theme);
      var url = new URL(location.href);
      url.searchParams.set('lang', this.state.lang);
      history.replaceState(null,'',url.toString());
    }catch(e){ window._diag_show('persist failed: '+e); }
  },

  setLang:function(l){
    this.state.lang = (l==='en'?'en':'he');
    document.documentElement.setAttribute('lang', this.state.lang==='he'?'he':'en');
    document.documentElement.setAttribute('dir',  this.state.lang==='he'?'rtl':'ltr');
    var sel = document.getElementById('lang_header'); if(sel){ sel.value = this.state.lang; }
  ...
  next:function(){
    var order=['entry','home','addSkip','history','insights','investments','summary'];
    var i=order.indexOf(this.state.screen);
    this.state.screen = order[Math.min(i+1,order.length-1)] || 'home';
    this.render();
  },
  ...
  exportCSV:function(){
    ...
    var header=this.state.lang==='he'?['תאריך','סכום','קטגוריה','תיאור']:['Date','Amount','Category','Description'];
    var lines=[header.join(',')];
    for(var i=0;i<rows.length;i++){
      var r=rows[i];
      lines.push([formatDate(r.date),(r.amount||0),csvQuote(r.category||''),csvQuote(r.description||'')].join(','));
    }
    var blob=new Blob(['\ufeff'+lines.join('\n')],{type:'text/csv;charset=utf-8;'});
    var a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='skipapp_history.csv'; a.click();
    URL.revokeObjectURL(a.href);
  },

  exportJSON:function(){
    var blob=new Blob([JSON.stringify(this.state.data,null,2)],{type:'application/json'});
    var a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='skipapp_history.json'; a.click();
    URL.revokeObjectURL(a.href);
  },

  // ===== יצוא PDF/HTML מודפס (מתוקן) =====
  exportPDF:function(period){
    try{
      var now=new Date();
      var lang=this.state.lang;
      var dir=(lang==='he'?'rtl':'ltr');
      var title=(lang==='he'?'סיכום חיסכון':'Savings Summary');

      var data=this.state.data.slice();
      function fmt(d){ return new Date(d); }

      var filtered=data;
      if(period==='month'){
        var m=now.getMonth(), y=now.getFullYear();
        filtered=data.filter(function(x){ var dx=fmt(x.date); return dx.getMonth()==m && dx.getFullYear()==y; });
      }else if(period==='quarter'){
        var q=Math.floor(now.getMonth()/3), yq=now.getFullYear();
        filtered=data.filter(function(x){ var dx=fmt(x.date); return Math.floor(dx.getMonth()/3)==q && dx.getFullYear()==yq; });
      }

      var total=0;
      for(var i=0;i<filtered.length;i++){ total+=(Number(filtered[i].amount)||0); }
      var count=filtered.length;
      var avg=count? total/count : 0;

      var html='<!DOCTYPE html><html lang="'+lang+'" dir="'+dir+'"><head><meta charset="utf-8"><title>'+title+'</title>'+
        '<style>body{font-family:Heebo,system-ui;margin:24px;color:#0f172a} h1{margin:0 0 8px} .muted{color:#64748B} '+
        'table{width:100%;border-collapse:collapse;margin-top:12px} th,td{border-bottom:1px solid #e2e8f0;padding:8px;text-align:'+(dir==='rtl'?'right':'left')+'} '+
        '.k{display:flex;gap:8px;margin-top:8px} .chip{border:1px solid #e2e8f0;border-radius:999px;padding:6px 10px}</style></head><body>';

      html+='<h1>'+title+'</h1>';
      html+='<p class="muted">'+(period==='month'?(lang==='he'?'חודש נוכחי':'Current Month'):(lang==='he'?'רבעון נוכחי':'Current Quarter'))+'</p>';
      html+='<div class="k">'+
              '<span class="chip">'+(lang==='he'?'מספר דילוגים':'Skips')+': '+count+'</span>'+
              '<span class="chip">'+(lang==='he'?'סך הכל':'Total')+': ₪'+total.toFixed(2)+'</span>'+
              '<span class="chip">'+(lang==='he'?'ממוצע':'Average')+': ₪'+avg.toFixed(2)+'</span>'+
            '</div>';

      html+='<table><thead><tr>'+
              '<th>'+(lang==='he'?'תאריך':'Date')+'</th>'+
              '<th>'+(lang==='he'?'סכום (₪)':'Amount (₪)')+'</th>'+
              '<th>'+(lang==='he'?'קטגוריה':'Category')+'</th>'+
              '<th>'+(lang==='he'?'תיאור':'Description')+'</th>'+
            '</tr></thead><tbody>';

      // *** לולאה תקינה – אין תו שגוי ***
      for(var j=0;j<filtered.length;j++){
        var r=filtered[j];
        html+='<tr><td>'+formatDate(r.date)+'</td>'+
      ...
      if(!w){ alert(lang==='he'?'פתח חלונות מוקפץ':'Enable pop-ups'); }
      if(w){ w.onload=function(){ try{ w.focus(); w.print(); }catch(e){} }; }
      setTimeout(function(){ URL.revokeObjectURL(url); },30000);
    }catch(e){
      window._diag_show('exportPDF failed: '+e);
    }
  },

  // ===== חישובי חודש =====
  monthSum:function(){
    var now=new Date(), m=now.getMonth(), y=now.getFullYear(), sum=0;
    for(var i=0;i<this.state.data.length;i++){
      var x=this.state.data[i], dx=new Date(x.date);
      if(dx.getMonth()==m && dx.getFullYear()==y){ sum += (Number(x.amount)||0); }
    }
    return sum;
  },

  // ===== שאילתות חיפוש ללשונית השקעות =====
  gQuery:function(q){
    var enc = encodeURIComponent(q);
    var hl  = this.state.lang==='he' ? 'he' : 'en';
    return 'https://www.google.com/search?q='+enc+'&hl='+hl;
  },

  // ===== גרף פשוט (canvas) =====
  renderChart:function(){
    var el=document.getElementById('chart'); if(!el) return;
    var ctx=el.getContext('2d'), w=el.width=el.clientWidth, h=el.height=320;
    ctx.clearRect(0,0,w,h);
    ctx.fillStyle=getComputedStyle(document.body).getPropertyValue('--cardbg'); ctx.fillRect(0,0,w,h);

    var data=this.state.data.slice().sort(function(a,b){ return new Date(a.date)-new Date(b.date); });
    var amounts=[], cum=[], s=0;
    for(var i=0;i<data.length;i++){ var v=Number(data[i].amount)||0; amounts.push(v); s+=v; cum.push(s); }
    var series=this.state.chartMode==='line'?cum:amounts;
    var max=10; for(var k=0;k<series.length;k++){ if(series[k]>max) max=series[k]; }

    var left=40,right=10,top=10,bottom=30,gw=w-left-right,gh=h-top-bottom;
    ...
    ctx.fillText((this.state.lang==='he'?'סכום (₪)':'Amount (₪)'),6,16);

    if(series.length===0){ ctx.fillText(this.state.lang==='he'?'אין נתונים':'No data', left+6, top+40); return; }

    var step=series.length>1?gw/(series.length-1):gw;
    if(this.state.chartMode==='line'){
      ctx.strokeStyle='#22d3ee'; ctx.lineWidth=2; ctx.beginPath();
      for(var m=0;m<series.length;m++){
        var x=left+m*step, y=h-bottom-(series[m]/max)*gh;
        if(m===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
      }
      ctx.stroke();
    } else {
      var barW=Math.max(4, gw/series.length*0.6);
      ctx.fillStyle='#818cf8';
      for(var n=0;n<series.length;n++){
        var xb=left+n*step-barW/2, yb=h-bottom-(series[n]/max)*gh;
        ctx.fillRect(xb,yb,barW,(series[n]/max)*gh);
      }
    }
  },
  ...
  // ===== רינדור מסכים =====
  render:function(){
    var el=document.getElementById('app'); var parts=[]; var t=this.t.bind(this);
    var total=0; for(var i=0;i<this.state.data.length;i++){ total += (this.state.data[i].amount||0); }
    function push(s){ parts.push(s); }

    try{
      if(this.state.screen==='entry'){
        push('<section class="card" style="text-align:center">');
        push('<div class="brandWrap"><h2 class="brandName">SKIPAPP · '+t('entry_title')+'</h2>images/play_logo.png</div>');
        push('<img src="images/hero.jpg" alt="SKIPAPP" style="width:100%;border-radius:'+t('entry_intro_1')+'</p>');
        ...
        var u=this.state.user||{firstName:'',age:'',goal:0,purpose:''};
        var opts=(strings[this.state.lang].purpose_opts||['השקעות','קניית נכס','חיסכון חכם','אחר']);
        ...
        var name=(this.state.user&&this.state.user.firstName)?this.state.user.firstName:'';
        var month=this.monthSum(); var avg=this.state.data.length?(total/this.state.data.length):0;
        push('<section class="card" style="text-align:center">');
        push('<div class="brandWrap"><h2 class="brandName">SKIPAPP'+(name?(' · '+t('welcome_home')+' '+name):'')+'</h2>images/play_logo.png</div>');
        push('images/hero.jpg');
        ...
        push('<div class="list" style="margin-top:12px">'+['add_skip','history','insights','investments','settings','charts'].map(function(s){return 'javascript:void(0)'+t(s)+'</a>';}).join('')+'</div>');
        ...
        var totalSaved=total;
        var msg = totalSaved>100 ? (this.state.lang==='he'?'כבר ניתן להתחיל לחסוך ולהשקיע.':'You can start saving and investing.')
                                 : (this.state.lang==='he'?'המשיכו בדילוגים קטנים כדי להגיע ליעד ולהתחיל להשקיע.':'Keep small skips to reach your goal and start investing.');
        push('<section class="card">');
        ...
        var q_he=['המושג ריבית דריבית','למה כדאי להשקיע מגיל צעיר','מה הם אפיקי ההשקעה הטובים ביותר','פירוש המושג מניות','פירוש המושג אג\"ח'];
        var q_en=['compound interest concept','why invest from a young age','best investment avenues','definition of stock','definition of bonds'];
        var queries=this.state.lang==='he'?q_he:q_en;
        push('<section class="card">');
        ...
        var count=this.state.data.length; var totalSaved=total;
        push('<section class="card" style="text-align:center">');
        push('<h2>'+(this.state.lang==='he'?'סיכום':'Summary')+'</h2>');
        push('<p class="hint">'+(this.state.lang==='he'?'מספר דילוגים':'Skips')+': '+count+'</p>');
        push('<p class="hint">'+t('total')+totalSaved.toFixed(2)+'</p>');
        push('<div class="row" style="margin-top:12px;justify-content:center"><button class="btn" onclick="App.goHome()">'+t('home')+'</button><button class="btn" onclick="App.next()">'+t('save_next')+'</button></div>');
        push('</section>');
      }
      ...
      window._diag_show('render failed: '+(err.message||err));
    }
  },
  ...
  load:function(){
    try{
      var urlLang=new URL(location.href).searchParams.get('lang');
      var storeLang=localStorage.getItem('skipapp_lang');
      this.state.lang = (urlLang||storeLang||'he');
    }catch(e){}
    document.documentElement.setAttribute('lang', this.state.lang==='he'?'he':'en');
    document.documentElement.setAttribute('dir    document.documentElement.setAttribute('dir',  this.state.lang==='he'?'rtl':'ltr');
    var sel=document.getElementById('lang_header'); if(sel){ sel.value=this.state.lang; }
